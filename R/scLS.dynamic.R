...
#'
#' @description
#' Run Lomb–Scargle periodogram via Astropy on a Seurat object.
#'
...
scLS.dynamic <- function(object, time.col, feature = NULL, assay = "RNA", slot = "data",
  ...
  process_features <- function(feats) {
    lapply(feats, function(feat) {
      ...
      if (!is.null(window.func)) {
        wf <- match.fun(window.func)  # "hanning" など
        wv <- wf(N)
      } else {
        wv <- rep(1, N)
      }
      ...
      freq.r <- py_to_r(freqs)
      fap.r  <- py_to_r(fap.vec)
      idx <- which.max(power)
  ...
  if (total_removed > 0)
    message(paste(total_removed, "NA/Inf removed across", n_feats, "features"))
